...
Lets apply the function "sumna" to detect columns with more than `r p*100` percent of missing values: 
...
In this case there are  `r di2[1]` columns with more than `r p*100` percent of missing values. if we sould like to eliminate those colums we apply the following:
...
```{r}
abs(descrCor[upper.tri(descrCor)]) > .999
```
...
Transform the Fully Paid category in number one, if the the  "Fully Paid" and zero if  "Charged Off" and insert the variable into the data frame, with the name "default".
```{r}
descrCor <-  cor(data4, na.rm = FALSE)
```


Review if some columns has nas. and replace if by the mean. 
```{r}

```


Here we estimate the mean

mean(data[,11],na.rm=T)
```{r}

```


To replace Nas with the mean:
data[,column][is.na(data[,column])]<-value that replace NA´s
...
The function na.omit(data) is another alternative, and it eliminates all the rows whit missing values. 
```{r}
